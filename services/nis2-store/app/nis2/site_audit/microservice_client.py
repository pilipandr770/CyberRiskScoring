...
=========================
REST client for the scanning backend behind Module 1 (risk-scanner) — was
previously a separate external Docker "black box" microservice; same
contract, now served by risk-scanner's /api/* routes (see
services/risk-scanner/app/routers/api.py and app/api_export.py in the
sibling project for the exact response shapes).

The backend runs full scanning (subfinder, nmap, nuclei, Shodan, local
CVE-cache matching) + Claude AI analysis.

Environment variables:
  NIS2_MICROSERVICE_URL      – risk-scanner base URL, e.g. http://risk-scanner:8000
  NIS2_MICROSERVICE_API_KEY  – must match risk-scanner's M3_API_KEY
  NIS2_MICROSERVICE_TIMEOUT  – per-request HTTP timeout in seconds (default 30)

Endpoints used:
  GET  /api/health                     – public health check
  POST /api/audit                      – start audit → {"job_id": "<uuid>"}
  GET  /api/audit/<id>                 – status: pending|running|done|failed
  GET  /api/audit/<id>/findings        – findings list
  GET  /api/audit/<id>/logs?after=<N>  – paginated log stream (always empty for now)
"""
...
def _start_audit(target: str, company: str) -> str:
    """POST /api/audit → microservice job_id.

    Was cast with int() when job_id was always a small integer from the old
    microservice. risk-scanner (the new backend behind this client) uses
    UUID strings as scan ids, so job_id is now an opaque string everywhere
    in this module — don't assume it's numeric."""
    r = requests.post(
        f"{_URL}/api/audit",
        headers=_headers(),
...
def _get_status(remote_id: str) -> dict:
...
def _get_findings(remote_id: str) -> List[dict]:
...
def _get_new_logs(remote_id: str, after: int = 0) -> List[dict]:
